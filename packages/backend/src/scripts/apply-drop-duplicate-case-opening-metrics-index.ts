 ...
 *
 * This script drops the duplicate idx_case_opening_metrics_user_id_btree index,
 * keeping only the standard idx_case_opening_metrics_user_id.
 */
...
async function applyDropDuplicateCaseOpeningMetricsIndex() {
  console.log('🚀 Applying Duplicate Index Fix for case_opening_metrics...\n');
  ...
  await executeSQL(
    ...
    'Drop duplicate idx_case_opening_metrics_user_id_btree index'
  );
  ...
  console.log('📝 Only the standard idx_case_opening_metrics_user_id index remains');
}
...
async function main() {
  await applyDropDuplicateCaseOpeningMetricsIndex();
  await verifyFix();
}
