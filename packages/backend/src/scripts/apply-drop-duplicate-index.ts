 ...
 *
 * This script drops the duplicate idx_game_history_user_id index,
 * keeping only the more descriptive idx_game_history_user_id_btree.
 */
...
async function applyDropDuplicateIndex() {
  console.log('🚀 Applying Duplicate Index Fix for game_history...\n');

  // Drop the duplicate index
  console.log('🗑️  Removing duplicate index...');
  await executeSQL(
    'DROP INDEX IF EXISTS idx_game_history_user_id;',
    'Drop duplicate idx_game_history_user_id index'
  );
  ...
  console.log('📝 Only the more descriptive idx_game_history_user_id_btree index remains');
}
