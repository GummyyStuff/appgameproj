 ...
 *
 * This script drops the duplicate idx_chat_messages_recent index,
 * keeping only the standard idx_chat_messages_created_at.
 */
...
async function applyDropDuplicateChatMessagesIndex() {
  console.log('🚀 Applying Duplicate Index Fix for chat_messages...\n');

  // Drop the duplicate index
  console.log('🗑️  Removing duplicate index...');
  await executeSQL(
    'DROP INDEX IF EXISTS idx_chat_messages_recent;',
    'Drop duplicate idx_chat_messages_recent index'
  );
  ...
  console.log('📝 Only the standard idx_chat_messages_created_at index remains');
}
...
async function main() {
  await applyDropDuplicateChatMessagesIndex();
  await verifyFix();
}
