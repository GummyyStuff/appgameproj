---
title: "Balance Constraints Documentation"
audience: developer
layer: backend
status: stable
tags: [balance, constraints, bug-fix]
...
**Root Cause:** The original implementation used a single database transaction for both balance deduction and case opening. If the case opening failed after balance deduction, it would leave the user in an inconsistent state.

### Solution Implemented

We introduced a two-phase commit pattern:
1. Validate sufficient balance
2. Deduct balance from user account
3. Process case opening
4. Commit or rollback all operations atomically
