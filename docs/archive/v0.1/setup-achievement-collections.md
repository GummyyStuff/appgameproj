---
title: "Achievement Collections Setup Guide"
audience: developer
layer: database
status: stable
tags: [appwrite, database, collections, achievements]
...
This guide explains how to set up the Appwrite collections required for the achievement system in the Tarkov Casino project.

## Architecture & Logic

The achievement system uses two Appwrite collections:
1. `achievement_definitions` - Stores metadata about available achievements
2. `user_achievements` - Tracks which achievements each user has earned

## Dependencies & Integration

This setup integrates with the Appwrite database service and requires proper security rules to be configured for both collections.

## Implementation Notes

Ensure that all attributes are properly indexed for optimal query performance. The achievementId field should be unique to prevent duplicate definitions.
