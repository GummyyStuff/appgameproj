...
layer: backend
status: stable
tags: [statistics, analytics, database, game-history]
...
The statistics system provides comprehensive tracking, analysis, and visualization of user gaming data for the Tarkov Casino Website. This system automatically records all game transactions and provides detailed analytics through various API endpoints.

## Architecture

### Core Components

1. **StatisticsService** - Handles advanced analytics and data processing
2. **API Routes** - RESTful endpoints for accessing statistics data
3. **Database Integration** - Records game history in Appwrite collections

### Data Flow

```
Game Play → DatabaseService.processGameTransaction() → Appwrite (game_history collection)
                                ↓
...
- Game type (roulette, blackjack, case_opening)
...
#### Game Type Breakdown
- Statistics per game type (roulette, blackjack, case_opening)
- Popularity rankings
...
| `user_id` | String | References Appwrite user |
| `game_type` | Enum | `roulette`, `case_opening` (DB schema) / `roulette`, `blackjack` (game engine) / `roulette`, `blackjack`, `case_opening` (stats routes) — see note below |
| `bet_amount` | Float | Wagered amount |
| `win_amount` | Float | Won amount |
| `result_data` | JSON | Game-specific result data |
...
### Game Type Inconsistency
There is a discrepancy in game type definitions across the codebase:
- **DB schema** (`types/database.ts`): `roulette`, `case_opening`
- **Game engine** (`core-engine.ts`): `roulette`, `blackjack`
- **Stats routes** (`statistics.ts`): `roulette`, `blackjack`, `case_opening`

This should be reconciled in a future refactor.
...
The statistics system provides a comprehensive foundation for tracking and analyzing user gaming behavior. With robust testing, proper error handling, and scalable architecture, it supports both current needs and future growth of the Tarkov Casino platform.